...
public class ServiceMqtt extends ServiceBindable implements MqttCallback {
    ...
    private void initMqttClient() {
        ...
        try {
            ...
            Log.v(LOG_TAG, "serverURI=" + serverURI);
            mqttClient = new MqttClient(serverURI, getClientId(), null);
    ...
    private boolean connect() {
        ...
        try {
            ...
            Log.d(LOG_TAG, "No error during connect");
    ...
    // Lint check dependencies with the wl.release() call.
    @Override
    public void connectionLost(Throwable t) {
        Log.e(LOG_TAG, "connectionLost(): error " + t.toString());
    ...
    private void changeState(Defaults.State.ServiceMqtt newState, Exception e) {
        Log.d(LOG_TAG, "ServiceMqtt state changed to: " + newState);
        state = newState;
    ...
    private void publish(final DeferredPublishable p) {
        pubHandler.post(new Runnable() {
            ...
            if (Looper.getMainLooper().getThread() == Thread.currentThread()) {
                Log.e(LOG_TAG, "PUB ON MAIN THREAD");
            }

            if (!isOnline(false) || !isConnected()) {
                Log.d(LOG_TAG, "pub deferred");
    ...
    private class DeferredPublishable extends MqttMessage {
        ...
        public void wait(LinkedList<DeferredPublishable> queue, Runnable onRemove) {
            if (timeoutHandler != null) {
                Log.d(LOG_TAG, "This DeferredPublishable already has a timeout set.");
                return;
            }
    ...
    private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context ctx, Intent intent)
        {
            ...
            if (isOnline(true) && !isConnected() && !isConnecting()) {
                Log.v(LOG_TAG, "NetworkConnectionIntentReceiver: triggering doStart(-1)");
                doStart(1);
            }
            wl.release();
        }
    ...
    public void messageArrived(String topic, MqttMessage message) throws Exception {
        ...
        //Log.v(LOG_TAG, "messageArrived: topic=" + topic + ", message=" + payloadStr);
        ...
        if (splitTopic[3].equals("controls")) {
            ...
            if (splitTopic.length == 5) { // Control value
                ...
                // TODO: Mr - Test for control value in room view
                if (splitTopic[6].equals("room")) { // view control on room view
                    // Ensure the device for the message exists
